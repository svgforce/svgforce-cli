...
export async function reactCommand(patterns: string[], opts: ReactOptions): Promise<void> {
  const files = await resolveSvgFiles(patterns);

  if (opts.dryRun) {
    log.info(`Would generate React component from ${chalk.bold(files.length)} SVG file(s):`);
  ...
  const spinner = ora(`Generating React component from ${files.length} SVG(s)...`).start();

  const icons = files.map((f) => ({ name: f.name, svg: f.svg }));

  const result = await apiRequest<ConvertResponse>('/convert/react-component', {
    method: 'POST',
    body: {
